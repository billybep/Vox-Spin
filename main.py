...
def build_full_email_html(body_content: str) -> str:
    ...
    return f"<!DOCTYPE html><html><body style='margin: 0; padding: 0; background-color: #030712; font-family: Arial, sans-serif;'><table width='100%' style='background-color: #030712; padding: 40px 0;'><tr><td align='center'><table width='600' style='background-color: #0a1f35; border-radius: 16px; border: 1px solid rgba(139, 92, 246, 0.2); padding: 35px; color: #f8fafc;'><tr><td>{header}{banner_html}{body_content}<br><hr style='border:0; border-top:1px solid rgba(255,255,255,0.05); margin:20px 0;'>{footer}</td></tr></table></td></tr></table></body></html>"
...
def generate_certificate(company_name: str, winner_date_str: str, week_num: int) -> str:
    ...
    try:
        title_font, name_font = ImageFont.truetype("arial.ttf", 42), ImageFont.truetype("arial.ttf", 64)
    except:
...
def register_participant(participant: Participant, background_tasks: BackgroundTasks):
    try:
        # VALIDASI KUNCI BATAS 20 BISNIS MINGGUAN
        pending_participants = list(db.collection("participants").where("status", "==", "pending").stream())
        if len(pending_participants) >= 20:
            raise fastapi.HTTPException(status_code=400, detail="The weekly limit of 20 active businesses has been reached. Please try again next week.")
...
async def import_csv(file: fastapi.UploadFile = fastapi.File(...)):
    try:
        ...
        imported_count = 0
        for row in reader:
            company_name = row.get("Brand Name") or row.get("companyName") or row.get("name")
            social_media = row.get("Instagram") or row.get("socialMedia") or row.get("social")
            ...
            doc_ref.set({
                ...
                "socialMedia": str(social_media).strip().lower() if social_media else "",
                "phone": str(phone).strip(),
                "email": row.get("Email") or row.get("email") or "",
                "website": row.get("Website") or row.get("website") or "",
                "tiktok": row.get("TikTok") or row.get("tiktok") or "",
                "facebook": row.get("Facebook") or row.get("facebook") or "",
                "address": row.get("Address") or row.get("address") or "",
                "niche": row.get("Industry") or row.get("niche") or "",
                "status": "pending",
        ...
        return {"status": "success", "message": f"Successfully imported {imported_count} records from CSV."}
    except Exception as e:
        raise fastapi.HTTPException(status_code=500, detail=str(e))
